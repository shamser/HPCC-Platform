...
static bool getResourceFromMappedFile(const char * filename, const byte * start_addr, size32_t & lenData, const void * & data, const char * type, unsigned id)
{
    ...
    {
        OERRLOG("Failed to extract resource %s: Does not appear to be a ELF binary", filename);
        return false;
    }
    if (hdr->e_ident[EI_CLASS] != ELFCLASS64)
    {
        OERRLOG("Failed to extract resource %s: Does not appear to be a ELF 64-bit binary", filename);
        return false;
    }
    ...
    {
        OERRLOG("Failed to extract resource %s: Does not include a section symbol table", filename);
        return false;
    }
    ...
    OERRLOG("Failed to extract resource %s: Does not include a matching entry", filename);
    return false;
#else
    // The first bytes are the ELF header
    ...
    {
        OERRLOG("Failed to extract resource %s: Does not appear to be a ELF 32-bit binary", filename);
        return false;
    }
...
extern bool getResourceFromFile(const char *filename, MemoryBuffer &data, const char * type, unsigned id)
{
    ...
    if (dllHandle == NULL)
    {
        OERRLOG("Failed to load library %s: %d", filename, GetLastError());
        return false;
    }
    ...
    if (fd == -1)
    {
        OERRLOG("Failed to load library %s: %d", filename, errno);
        return false;
    }
